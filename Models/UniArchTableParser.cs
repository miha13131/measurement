...
namespace DeviceMeasurementsApp.Models
{
    public static class UniArchTableParser
    {
        public static UniArchTable ParseMinuteTable(byte[] archBytes, DateTime startLocal, int periodMs = 60000, int maxRows = 300, int? expectedColumns = null)
        {
            ...
            {
                table.Columns.Add(new UniColumn(Index: i, Name: $"Value{i + 1}", Unit: ""));
            }
            ...
            for (int r = 0; r < rows; r++)
            {
                ...
                {
                    uint be = BinaryPrimitives.ReadUInt32BigEndian(archBytes.AsSpan(baseOffset + (c + serviceFloatsPerRow) * 4, 4));
        ...
        private static int InferRawFloatsPerRow(byte[] archBytes)
        {
            ...
            for (int i = 0; i < maxFloatsToScan; i++)
            {
                ...
                if (be == marker) markerFloatIndexes.Add(i);
            }

            if (markerFloatIndexes.Count < 3)
                return 47;

            var topDiffs = markerFloatIndexes.Zip(markerFloatIndexes.Skip(1), (a, b) => b - a).Where(d => d > 1).GroupBy(d => d).Select(g => new { Diff = g.Key, Count = g.Count() }).OrderByDescending(x => x.Count)
                .ThenByDescending(x => x.Diff).Take(3).ToArray();
            ...
            var best = candidates.Select(c => new { Candidate = c, Score = Score(c) }).Where(x => x.Score >= 0).OrderByDescending(x => x.Score).ThenBy(x => x.Candidate).FirstOrDefault();
